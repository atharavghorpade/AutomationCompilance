    ...
    public Result execute(String jsCode, String config, String ruleId) {
        ...
        try (Context context = Context.newBuilder("js")
            ...
            if (checkFunction == null || !checkFunction.canExecute()) {
                log.warn("Execution failed for ruleId={} because check(config) was not found or not executable", ruleId);
            ...
            if (exception.isSyntaxError()) {
                log.warn("Syntax error while executing JS for ruleId={}: {}", ruleId, exception.getMessage());
            ...
            log.warn("Invalid config while executing ruleId={}: {}", ruleId, exception.getMessage());
